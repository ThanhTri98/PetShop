...
//                    Toast.makeText(PaymentOptionActivity.this, "VNPay", Toast.LENGTH_SHORT).show();
    ...
    public static String concatAllFields(Map fields) throws UnsupportedEncodingException {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == AppMoMoLib.getInstance().REQUEST_CODE_MOMO && resultCode == -1) {
            ...
            if (resultCode == RESULT_OK) {
                Log.e("PaymentOption_value_238", RESULT_OK + "");
                if (data.getStringExtra("message").equals("OK")) {
                    Intent i = new Intent();
                    //getApplicationContext(), CoinsActivity.class
                    i.putExtra("payments", "Payment with VNPAY");
                    Log.e("PaymentOption_value_242", data.getLongExtra("value", 0) + "");
                    i.putExtra("value", String.valueOf(data.getLongExtra("value", 0)));
                    i.putExtra("isSu", true);
                    setResult(99, i);

                    finish();
                } else {
                    Log.e("PaymentOption_value_248", "data message not ok");
                }
            }
        }
    }
